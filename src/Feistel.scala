...
object Feistel extends App{
  ...
  def encrypt(key: Long, message: Array[Byte]): Array[Byte] = {
    ...
    buffer.clear()
    var last = iv
    buffer.asLongBuffer().put((for(a <- arr) yield { last = encryptLong(key, last ^ a); last}).toArray)
    buffer.array()
  }
  ...
  def decrypt(key: Long, cipher: Array[Byte]): Array[Byte] = {
    ...
    var tmp = 0L
    buffer.clear()
    buffer.asLongBuffer().put((for(a <- arr) yield {tmp = last; last = a; decryptLong(key, a) ^ tmp }).toArray)
    buffer.array()
  }

  val key = 111857465
  val en = encrypt(key, "00000000000000000000000".getBytes)
